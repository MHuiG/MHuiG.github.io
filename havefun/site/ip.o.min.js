function checkp() {
	var checkExistIP = setInterval(function() {
		if (typeof window.MV == 'undefined') return;
		if (typeof window.MV.ip == 'undefined') return;
		clearInterval(checkExistIP);
		try {
			window.MV.MC.util.Visitor();
		} catch(e) {}
	},
	100)
};
function ipa(url, fun) {
	$.ajax({
		url: url,
		type: 'GET',
		data: {
			type: "json"
		},
		success: function(data) {
			window.MV.region = data;
			window.MV.ip = data.data.myip;
			checkp()
		},
		error(data) {
			checkp();
			var p = new fun();
			p.start();
		}
	})
};
function ipb(url, fun) {
	$.ajax({
		url: url,
		type: 'GET',
		success: function(data) {
			window.MV.region = data;
			window.MV.region.data.country = data.data.city;
			window.MV.ip = data.data.ip;
			checkp();
		},
		error(data) {
			checkp();
			var p = new fun();
			p.start();
		}
	})
};
function ipc(url) {
	$.ajax({
		url: url,
		type: 'GET',
		success: function(data) {
			window.MV.region = data;
			window.MV.region.data.country = data.data.city;
			window.MV.ip = data.data.ip;
			checkp();
		},
		error(data) {
			checkp();
			kill();
		}
	})
};
var checkExistJQ = setInterval(function() {
	if (typeof jQuery == "undefined") return;
	clearInterval(checkExistJQ);
	var fN = function() {
		this.start = function() {
			ipc('https://ip.mhuig.top/getIp')
		};
	};
	var f14 = function() {
		this.start = function() {
			ipb('https://i.p.mhuig.top/getIp', fN);
		};
	};
	var f13 = function() {
		this.start = function() {
			ipb('https://ip.f.mhuig.top/getIp', f14);
		};
	};
	var f12 = function() {
		this.start = function() {
			ipb('https://ip.e.mhuig.top/getIp', f13);
		};
	};
	var f11 = function() {
		this.start = function() {
			ipb('https://ip.d.mhuig.top/getIp', f12);
		};
	};
	var f10 = function() {
		this.start = function() {
			ipb('https://ip.c.mhuig.top/getIp', f11);
		};
	};
	var f9 = function() {
		this.start = function() {
			ipb('https://ip.b.mhuig.top/getIp', f10);
		};
	};
	var f8 = function() {
		this.start = function() {
			ipb('https://ip.a.mhuig.top/getIp', f9);
		};
	};

	var f7 = function() {
		this.start = function() {
			ipb('https://info.mhuig.top/getIp', f8);
		};
	};
	var f6 = function() {
		this.start = function() {
			ipa('https://ip.ss.zxinc.org/info.php', f7);
		};
	};
	var f5 = function() {
		this.start = function() {
			ipa('https://ip.lsy.cn/info.php', f6);
		};
	};
	var f4 = function() {
		this.start = function() {
			ipa('https://ip.zxinc.org/info.php', f5);
		};
	};
	var f3 = function() {
		this.start = function() {
			ipa('https://ip.ss.zxinc.org/api.php', f4);
		};
	};
	var f2 = function() {
		this.start = function() {
			ipa('https://ip.lsy.cn/api.php', f3);
		};
	};
	var f1 = function() {
		this.start = function() {
			ipa('https://ip.zxinc.org/api.php', f2);
		};
	};
	var p = new f1();
	p.start();
},
100);